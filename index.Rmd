...
**Careful: It's not clear that the below results are meaningful.
Those are statistics for *continuous* variables, which is perhaps not appropriate here.**
...
<!-- TODO really weird observation: the variable pairs right off the diagonal appear to be more highly correlated than the rest; hypothesis would be that this is because this is the order of the vars in the questionnaire; people were just lazy.  -->
<!-- might be interesting to find a statistical test for this -->
<!-- it gets worse: if you look at the highly correlated variable pairs asked right after one another in the questionnaire, *some* of them may be considered to substantially overlapping or plausibly correlated, such as `balance_loc` and `balance_time` (at .7). -->
<!-- But the second highest correlations can be found between `deadline` and `quantity` (at .6), which overlap in content (to have some influence at work), but also simply in their *wording*. -->
<!-- In the absence of a randomized question order, there is of course no evidence that the observed correlations of consecutive questions is an artefact of wording or order, but the conspicuous pattern of high correlations leaves one highly suspicious of their validity. -->
<!-- There is, in any event, no reason *why* we should see high correlations between consecutive items. -->
<!-- The absence of randomized question order here strictly limits any conclusions that may be drawn from the data or its inherent structure. -->
<!-- TODO correlate the correlations of variables with their proximity on the questionnaire -->
...
```{r}
paran_work <- paran::paran(
  iterations = 10000, 
  centile = 95, 
  mat = cor(crowddata$exp_work, use = "pairwise.complete.obs"), 
  n = 230
)
r2_work <- sum(paran_work$Ev[1:paran_work$Retained])/28
pca_work <- psych::principal(r = crowddata$exp_work, nfactors = paran_work$Retained, rotate = "none")
```
