...
`r nrow(crowddata)` Cordworker beantworteten den Fragebogen, davon jeweils `r table(crowddata$study)` rekrutiert über den Anwendungstester [Applause](https://www.applause.com), die Innovations- und Marktforschungsplatform  [Atizo](https://www.atizo.com) sowie den generischen Anbieter [Crowdguru](https://www.crowdguru.de).
...
Im Mittelpunkt der unten stehenden Analyse stehen allerdings die Antworten auf eine `r ncol(crowddata$exp_crowd)`-Item Batterie zur Erwartungen an Crowdarbeit.
<!-- TODO need source for the item battery -->
Diese Einstellungen mittels einer 5-stufigen Likertskala erhoben, und in ähnlichen Formulierungen auch in Bezug auf konventionelle Erwerbsarbeit abgefragt.
...
```{r battery, fig.cap="Erwartungen an Crowdarbeit", fig.width=12, fig.height=10}
interesting <- NULL
interesting$pay <- c("wage_organisation", "wage")
...
plot_battery(m = crowddata$exp_crowd, items = unlist(interesting), width = 35, lang = "de")
```

```{r difference, fig.cap="Erwartungen an Reguläre Beschäftigung - Crowdwork", fig.width=12, fig.height=10}
m <- crowddata$exp_work - crowddata$exp_crowd
plot_battery(m = m, items = unlist(interesting), width = 35, lang = "de")
```
